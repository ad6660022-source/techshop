      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden transition-all duration-200 group-hover:shadow-md group-hover:-translate-y-1 h-full flex flex-col">
        ...
        >
          ...
          {images.length > 1 && (
            <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              {images.map((_, i) => (
                <div key={i} className={cn("w-1.5 h-1.5 rounded-full transition-colors", i === imgIdx ? "bg-blue-600" : "bg-gray-300")} />
              ))}
            </div>
          )}

          <div className="absolute top-2 left-2 flex flex-col gap-1">
            {discount > 0 && <span className="bg-red-500 text-white text-xs font-bold px-2 py-0.5 rounded-md">-{discount}%</span>}
            {product.isNew && <span className="bg-green-500 text-white text-xs font-bold px-2 py-0.5 rounded-md">Новинка</span>}
            {product.stock === 0 && <span className="bg-gray-500 text-white text-xs font-bold px-2 py-0.5 rounded-md">Нет в наличии</span>}
          </div>
          ...
          <div className="absolute top-2 right-2 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-150">
            ...
            <button onClick={handleCompare}
              className={cn("w-8 h-8 rounded-lg flex items-center justify-center shadow-sm transition-colors",
                inCompare ? "bg-blue-500 text-white" : "bg-white text-gray-600 hover:bg-blue-50 hover:text-blue-500")}
              title="Сравнить">
        ...
        <div className="p-4 flex flex-col flex-1">
          ...
          <div className="mb-3">
            ...
            {product.stock > 0 && product.stock <= 5 && <p className="text-xs text-orange-600 font-medium mt-0.5">Осталось {product.stock} шт.</p>}
          </div>

          <button onClick={handleAddToCart} disabled={product.stock === 0}
            className={cn("w-full h-9 rounded-lg text-sm font-medium flex items-center justify-center gap-2 transition-all duration-150",
              product.stock > 0 ? "bg-blue-600 text-white hover:bg-blue-700 active:scale-[0.98]" : "bg-gray-100 text-gray-400 cursor-not-allowed")}>
