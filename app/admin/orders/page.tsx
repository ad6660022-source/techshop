  ...
  return (
    <div>
      ...
      <div className="space-y-4">
        ...
        {orders.map((order) => {
          const isRefundPending = order.status === "CANCELLED" && order.paymentStatus === "PAID";
          ...
          return (
            <div
              ...
              className={`bg-white rounded-2xl border p-6 ${
                isRefundPending ? "border-orange-200 bg-orange-50/30" : "border-gray-100"
              }`}
            >
              <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                ...
                <div className="flex flex-col items-start lg:items-end gap-3 shrink-0">
                  <div className="text-right">
                    ...
                    <p className={`text-xs font-medium mt-0.5 ${isRefundPending ? "text-orange-600" : "text-gray-400"}`}>
                      {order.paymentStatus === "PAID" && order.status !== "CANCELLED" ? "Оплачен" : ""}
                      {order.paymentStatus === "PAID" && order.status === "CANCELLED" ? "Оплачен - нужен возврат" : ""}
                      {order.paymentStatus === "PENDING" ? "Ожидает оплаты" : ""}
                      {order.paymentStatus === "REFUNDED" ? "Возврат выполнен" : ""}
                      {order.paymentStatus === "FAILED" ? "Оплата не прошла" : ""}
                    </p>
                  </div>
                  <OrderStatusUpdater
                    orderId={order.id}
